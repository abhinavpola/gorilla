...
def encode_question(question, api_name, append_system=True):
...
def get_response(get_response_input, api_key):
    ...
    try:
        if "gpt" in model:
            ...
            question, system_prompt = encode_question(
                question, api_name, append_system=False
            )
...
def write_result_to_file(result, output_file):
    ...
    with file_write_lock:
        with open(output_file, "a") as outfile:
            ...
            outfile.write("\n")
            print(f"Wrote to file at {output_file}")
...
if __name__ == "__main__":
    ...
    with mp.Pool(args.num_workers) as pool:
        ...
        for result in results:
            response = result.get()  # This waits for and gets the actual result
            if response:
                write_result_to_file(response, args.output_file)
